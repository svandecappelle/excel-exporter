...
export class XlsxExporterService implements Exporter<ExcelOptions> {
    ...
    export(rows: any[], options: ExcelOptions) {
        ...
        const arrayRows = lodash.map(rows, (d: any) => lodash.map(options.columns, (col: any) => d[col.key]));
        worksheet.addTable({
            name: 'MyTable',
            ref: 'A1',
            headerRow: true,
...
export default (context: any, inject: (arg0: string, arg1: (items: Array<any>, opts: ExcelOptions) => void) => void) => {
